import pandas as pd
import init
from init import getNode, updateDanceDfs
                ...
                for heat_index, each in enumerate(heat_list.getRostersList()):
                    ...
                    for i, every in enumerate(placed_inst):
                        ...
                        if conflict_inst in every:
                            ...
                            inst_dup = True
                    if inst_dup:
                        resolverconflict = ResolverConflictItemSingle(3, nconflict_div, heat_index, nconflict_room, nconflict_index, instructors_in_heat, singles_in_heat, conflict_inst, conflict, [])
                        resolverLog.addConflict(resolverconflict)
                        dup = True
                    ...
                    if sing_dup:
                        resolverconflict = ResolverConflictItemSingle(4, nconflict_div, heat_index, nconflict_room, nconflict_index, instructors_in_heat, singles_in_heat, conflict_contestant, conflict, [])
                        resolverLog.addConflict(resolverconflict)
                        dup = True
                    # Check this heats singles
                    ...
                    if sing_dup:
                        resolverconflict = ResolverConflictItemSingle(5, nconflict_div, heat_index, nconflict_room, nconflict_index, instructors_in_heat, singles_in_heat, conflict_contestant, conflict, [])
                        resolverLog.addConflict(resolverconflict)
                        dup = True
                    if dup:
                        continue
                        ...
                        for contestant, inst in zip(placed_sing[swapping_room], placed_inst[swapping_room]):
                            for i, singles in enumerate(heat.getSingles()):
                                if contestant in singles:
                                    ...
                                    resolverconflict = ResolverConflictItemSingle(4, conflict_div, heat_index, swapping_room, placed_sing[swapping_room].index(contestant), instructors_in_heat, singles_in_heat, contestant, conflict, [])
                                    resolverLog.addConflict(resolverconflict)
                            for i, instructors in enumerate(heat.getInstructors()):
                                if inst in instructors:
                                    ...
                                    resolverLog.addConflict(resolverconflict)
                                    # conflict to check the previous heat entry nth conflict
                                    resolverconflict = ResolverConflictItemSingle(3, conflict_div, heat_index, swapping_room, placed_inst[swapping_room].index(inst), instructors_in_heat, singles_in_heat, inst, conflict, [])
                                    resolverLog.addConflict(resolverconflict)
                            for i, couples in enumerate(heat.getCouples()):
                                if contestant in couples:
                                    ...
                                    resolverconflict = ResolverConflictItemSingle(5, nconflict_div, -1, nconflict_room, nconflict_index, placed_inst[swapping_room], placed_sing[swapping_room], contestant, conflict, [])
                                    resolverLog.addConflict(resolverconflict)
                                if inst in couples:
                                    dup = True
                                    raise Exception("Instructor is in Couple list ")
                    ...
                    for i, every in enumerate(singles_in_heat):
                        if each in every:  # if room has a possible match for this instructor
                            ...
                            for fix in possible_changes:
                                ...
                                if presolved != solved:
                                    log.clearConflict(conflict_inst, -1)
                                    print("Resolved Conflict by swapping", conflict_contestant, conflict_inst, "and", contestant_data.loc[0, contestant_col], contestant_data.loc[0, inst_col], heat_index)
                                    return 1
                    ...
                    for heat_index, each in enumerate(heat_list.getRostersList()):
                        ...
                        for i, every in enumerate(placed_inst):
                            ...
                            if conflict_inst in every:
                                ...
                                inst_dup = True
                        if inst_dup:
                            resolverconflict = ResolverConflictItemSingle(3, nconflict_div, heat_index, nconflict_room, nconflict_index, instructors_in_heat, singles_in_heat, conflict_inst, conflict, [check_in_heat, free_inst])
                            resolverLog.addConflict(resolverconflict)
                            dup = True
                        # Check this heats singles
                        placed_sing = each.getSingles()
                        ...
                        for i, every in enumerate(placed_sing):
                            ...
                            if conflict_inst in every:
                                ...
                                raise Exception("Instructor is in Singles list ")
                        if sing_dup:
                            resolverconflict = ResolverConflictItemSingle(4, nconflict_div, heat_index, nconflict_room, nconflict_index, instructors_in_heat, singles_in_heat, conflict_contestant, conflict, [check_in_heat, free_inst])
                            resolverLog.addConflict(resolverconflict)
                            dup = True
                        # Check this heats singles
                        placed_coup = each.getCouples()
                        ...
                        for i, every in enumerate(placed_coup):
                            ...
                            if conflict_inst in every:
                                raise Exception("Instructor is in Couple list ")
                        if sing_dup:
                            resolverconflict = ResolverConflictItemSingle(5, conflict_div, heat_index, nconflict_room, nconflict_index, instructors_in_heat, singles_in_heat, conflict_contestant, conflict, [check_in_heat, free_inst])
                            resolverLog.addConflict(resolverconflict)
                            dup = True
                        if dup:
                            continue
                                ...
                                for i, singles in enumerate(heat.getSingles()):
                                    if contestant in singles or contestant in possible_matches:  # If the contestant is in possible matches then it won't solve anything
                                        ...
                                        if contestant in singles:
                                            ...
                                            resolverconflict = ResolverConflictItemSingle(4, nconflict_div, -1, nconflict_room, nconflict_index, placed_inst[swapping_room], placed_sing[swapping_room], contestant, conflict, [check_in_heat, free_inst])
                                            resolverLog.addConflict(resolverconflict)
                                        # To check prev heat with nth conflict
                                        resolverconflict = ResolverConflictItemSingle(4, conflict_div, heat_index, swapping_room, placed_sing[swapping_room].index(contestant), instructors_in_heat, singles_in_heat, contestant, conflict, [check_in_heat, free_inst])
                                        resolverLog.addConflict(resolverconflict)
                                for i, instructors in enumerate(heat.getInstructors()):
                                    if inst in instructors or inst == free_inst:  # If the instructor is also the one trying to be use it will solve nothing
                                        ...
                                        if inst in instructors:
                                            ...
                                            resolverLog.addConflict(resolverconflict)
                                        # To check prev heat with nth conflict
                                        resolverconflict = ResolverConflictItemSingle(3, conflict_div, heat_index, swapping_room, placed_inst[swapping_room].index(inst), instructors_in_heat, singles_in_heat, inst, conflict, [check_in_heat, free_inst])
                                        resolverLog.addConflict(resolverconflict)
                                for i, couples in enumerate(heat.getCouples()):
                                    if contestant in couples or contestant in possible_matches:
                                        ...
                                        nconflict_div = heat.getDiv()[nconflict_room]
                                        # Conflict to check og heat of nth conflict
                                        resolverconflict = ResolverConflictItemSingle(5, nconflict_div, -1, nconflict_room, nconflict_index, placed_inst[swapping_room], placed_sing[swapping_room], contestant, conflict, [check_in_heat, free_inst])
                                        resolverLog.addConflict(resolverconflict)
                                    if inst in couples:
                                        dup = True
                                        raise Exception("Instructor is in Couple list ")
        ...
        if conflict.getCode() == 1 or conflict.getCode() == 3:
            ...
            for possible_inst in conflict_entry.loc[0, "Instructor Dancer #'s"]:
                ...
                if possible_inst != conflict_inst and possible_inst not in instructors_list:
                    ...
                    try:
                        resolverconflict = ResolverConflictItemSingle(1, nconflict_div, conflict_heat_index, nconflict_room, nconflict_index, instructors_in_heat, singles_in_heat, possible_inst, conflict, [])
                    ...
                    for contestant, inst in zip(placed_sing[swapping_room], placed_inst[swapping_room]):
                        for i, singles in enumerate(singles_to_compare):
                            if contestant in singles:
                                ...
                                resolverconflict = ResolverConflictItemSingle(4, nconflict_div, conflict_heat_index, nconflict_room, nconflict_index, placed_inst[swapping_room], placed_sing[swapping_room], contestant, conflict, [])
                                resolverLogn.addConflict(resolverconflict)
                        ...
                        for i, instructors in enumerate(conflict_heat.getInstructors()):
                            if inst in instructors:
                                ...
                                resolverconflict = ResolverConflictItemSingle(3, nconflict_div, conflict_heat_index, nconflict_room, nconflict_index, placed_inst[swapping_room], placed_sing[swapping_room], inst, conflict, [])
                                resolverLogn.addConflict(resolverconflict)
                                # To check prev heat with nth conflict
                                resolverconflict = ResolverConflictItemSingle(3, conflict_div, heat_index, swapping_room, placed_inst[swapping_room].index(inst), instructors_in_heat, singles_in_heat, inst, conflict, [])
                                resolverLogn.addConflict(resolverconflict)
                        for i, couples in enumerate(conflict_heat.getCouples()):
                            if contestant in couples:
                                ...
                                nconflict_div = conflict_heat.getDiv()[nconflict_room]
                                # Conflict in the og heat
                                resolverconflict = ResolverConflictItemSingle(5, nconflict_div, conflict_heat_index, nconflict_room, nconflict_index, placed_inst[swapping_room], placed_sing[swapping_room], contestant, conflict, [])
                                resolverLogn.addConflict(resolverconflict)
                                # To check prev heat with nth conflict
                                resolverconflict = ResolverConflictItemSingle(5, conflict_div, heat_index, swapping_room, placed_coup[swapping_room].index(contestant), instructors_in_heat, singles_in_heat, contestant, conflict, [])
                                resolverLogn.addConflict(resolverconflict)
                            if inst in couples:
                                dup = True
                                raise Exception("Instructor is in Couple list ")
                    ...
                    for heat_index, each in enumerate(heat_list.getRostersList()):
                        ...
                        for i, every in enumerate(placed_coup):
                            ...
                            if conflict_inst in every:
                                raise Exception("Instructor is in Couple list ")
                        if sing_dup:
                            resolverconflict = ResolverConflictItemSingle(5, nconflict_div, heat_index, nconflict_room, nconflict_index, instructors_in_heat, singles_in_heat, conflict_contestant, conflict, [check_in_heat, free_inst])
                            resolverLogn.addConflict(resolverconflict)
                            dup = True
                        if dup:
                            continue
                            ...
                            for contestant, inst in zip(placed_sing[swapping_room], placed_inst[swapping_room]):
                                for i, singles in enumerate(conflict_heat.getSingles()):
                                    if contestant in singles or contestant in possible_matches:  # If the contestant is in possible matches then it won't solve anything
                                        ...
                                        if contestant in singles:
                                            ...
                                            nconflict_div = conflict_heat.getDiv()[nconflict_room]
                                            # Conflict to check og heat of nth conflict
                                            resolverconflict = ResolverConflictItemSingle(4, nconflict_div, conflict_heat_index, nconflict_room, nconflict_index, placed_inst[swapping_room], placed_sing[swapping_room] , contestant, conflict, [check_in_heat, free_inst])
                                            resolverLogn.addConflict(resolverconflict)
                                        # To check prev heat with nth conflict
                                        resolverconflict = ResolverConflictItemSingle(4, conflict_div, heat_index, swapping_room, placed_sing[swapping_room].index(contestant), instructors_in_heat, singles_in_heat, contestant, conflict, [check_in_heat, free_inst])
                                        resolverLogn.addConflict(resolverconflict)
                                ...
                                for i, couples in enumerate(conflict_heat.getCouples()):
                                    if contestant in couples or contestant in possible_matches:
                                        ...
                                        resolverLogn.addConflict(resolverconflict)
                                        # To check prev heat with nth conflict
                                        resolverconflict = ResolverConflictItemSingle(5, conflict_div, heat_index, swapping_room, placed_coup[swapping_room].index(contestant), instructors_in_heat, singles_in_heat, contestant, conflict, [check_in_heat, free_inst])
                                        resolverLogn.addConflict(resolverconflict)
                                    if inst in couples:
                                        dup = True
                                        raise Exception("Instructor is in Couple list ")
                        ...
                        if conflict_heat_index == -1:
                            ...
                            if tmp.loc[0, inst_col] in instructors_available_for_heat[conflict_room]:
                                instructors_available_for_heat[conflict_room].remove(tmp.loc[0, inst_col])
                        print("Resolved " + str(order) + " order conflict by swapping", conflict_contestant, conflict_inst, "and", conflict_heat.getSingles()[conflict_room][conflict_index], conflict_heat.getSingles()[conflict_room][conflict_index], "in heats", heat_index, conflict_heat_index)
                        return 1
